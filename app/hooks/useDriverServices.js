...
export const useDriverServices = ({
  ...
  const controlPoolingService = async (start) => {
    ...
    try {
      console.log(
        start
          ? "🚀 Starting pooling service..."
          : "🛑 Stopping pooling service..."
      );

      if (RidePoolingModule?.controlPoolingService) {
        await RidePoolingModule.controlPoolingService(
          start,
          driverId,
          token,
          API_URL_APP
        ...
        console.log(
          start ? "✅ Pooling service started" : "✅ Pooling service stopped"
        );
        return true;
      } else {
        console.warn(
          "⚠️ controlPoolingService method not found on RidePoolingModule"
        );
        return false;
      }
    } catch (error) {
  ...
  const checkServiceStatus = async () => {
    try {
      ...
      if (
        Platform.OS === "android" &&
        FloatingWidgetService?.isFloatingIconVisible
      ) {
