...
export default function CreateQuotations() {
  ...
  const [pricingMode, setPricingMode] = useState("detailed");
  ...
  const [totalAmount, setTotalAmount] = useState(""); // simplified mode

  // New Fields
  const [discount, setDiscount] = useState("0"); // Optional discount in ₹
  const [gstAmount, setGstAmount] = useState("0"); // 5% GST (auto-calculated)
  ...
  // Calculate subtotal & GST (5%) whenever relevant fields change
  useEffect(() => {
    let subtotal = 0;
    ...
    if (pricingMode === "detailed") {
      ...
      const extras = extraCharges.reduce(
      ...
      subtotal = days * perDay + toll + extras + addCharges;
    } else {
    ...
    const gst = (afterDiscount * 0.05).toFixed(2); // 5% fixed
  ...
  const validateForm = () => {
    ...
    if (pricingMode === "detailed") {
      validations.push({
        value: perDayCharges && parseFloat(perDayCharges) > 0,
        message: "Valid per day charges required",
      });
    } else {
      validations.push({
        value: totalAmount && parseFloat(totalAmount) > 0,
        message: "Valid total amount required",
      });
    }

    for (const v of validations) {
      if (!v.value) {
        showAlert("error", "Validation Error", v.message);
        return false;
      }
    }
    return true;
    ...
    try {
      ...
      const quotationData = {
        ...
        bill_to: {
          ...
          address: customerAddress.trim(), // NEW
        },
        trip_type: tripType,
        ...
        summary: {
          ...
          discount: parseFloat(discount) || 0, // NEW
          gst_amount: parseFloat(gstAmount) || 0, // NEW
        },
  ...
  const updateExtraCharge = (i, field, val) => {
          ...
          <View style={styles.tripTypeContainer}>
            ...
            >
              <MaterialCommunityIcons
                name="arrow-right"
                size={20}
                color={tripType === "one_way" ? "#000" : "#FF1744"}
              />
              <Text
                style={[
                  styles.tripTypeText,
        ...
        <View style={styles.section}>
          ...
          <View style={styles.pricingModeContainer}>
            ...
            <TouchableOpacity
              style={[
                styles.pricingModeButton,
                pricingMode === "simplified" && styles.pricingModeButtonActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.pricingModeText,
                  pricingMode === "simplified" && styles.pricingModeTextActive,
                ]}
              >
                Simplified (Total)
              </Text>
          ...
          {!company?.gst_number ? (
            <View style={styles.row}>
              <View style={{ flex: 1 }}>
                ...
                <TextInput style={styles.input} value={gstAmount} editable={true} onChangeText={setGstAmount} keyboardType="numeric" />
                <Text style={styles.helper}>Auto-calculated at 5%</Text>
              </View>
            </View>
          ) : null}
        </View>
