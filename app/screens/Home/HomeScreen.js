...
export default function HomeScreen() {
  ...
  const fetchRides = async (
    ...
    try {
      ...
      const [taxiRes, driverRes] = await Promise.all([
        fetchWithRetry(() =>
          fetch(
            `${API_URL_APP}/api/v1/Fetch-Near-By-Taxi-Safar-Rides?page=${taxiPageNum}&limit=10`,
            {
        ...
        fetchWithRetry(() =>
          fetch(
            `${API_URL_APP}/api/v1/fetch-nearby-rides?page=${driverPageNum}&limit=10`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          ).then((res) => res.json())
  ...
  return (
    <Layout state={state}>
      <DriverMap isRefresh={refreshing} />
      <Categories isRefresh={refreshing} />
          ...
          renderItem={() => (
            <View>
              ...
              <FlatList
                ...
                showsVerticalScrollIndicator={false}
                style={
                  driverPosts.length > 0 ? styles.driverSectionMargin : null
                }
              />
            </View>
          )}
